function moment_animation(beam, showDivs, exportGif)
    ...
    V_dead = f_dead(x);
    f_live_total = beam.moment_loadBetween(beam.load.live, 0, beam.total_length);
    ...
    gifFile = fullfile(resultsFolder, 'moment_animation.gif');

    for i = 1:length(n_values)
        n = n_values(i);
        [~, envelope] = moment_envelope(beam, n);
        clf;
        hold on;
        ...
        xlabel('Longitud (m)');
        ylabel('Cortante (kN)');
        title(sprintf('Envolvente de Momento (n = %d)', n));
